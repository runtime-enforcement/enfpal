...
ENFPAL  = Path("./_build/default/bin/enfpal.exe")
...
class Test:
    ...
    def _make_command(self) -> None:
        command : List[str] = [str(ENFPAL), "-sig", str(self.sig), "-formula", str(self.formula), "-log", str(self.log)]
        if self.func:
            command += ["-func", str(self.func)]
        self.command = command

    def _run_enfpal(self) -> subprocess.CompletedProcess[str]:
        try:
    ...
    def run(self) -> pd.Series:
        ...
        start_time = time()
        completed_process = self._run_enfpal()
